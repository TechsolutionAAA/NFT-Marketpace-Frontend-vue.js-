  ...
  if(elem.length > 0) {
    elem.forEach(item => {
      item.addEventListener("change", function(){
        ...
        var allowedExtensions  = ["jpg", "png", "gif", "webp", "mp4", "mp3", "glb", "gltf", "4ds"];
        ...
        if(!allowedExtensions.includes(fileExtension)) {
          alert(extTxt + " file type not allowed, Please upload .jpg, .png, .gif, .glb, .gltf, .4ds  .webp, .mp4 or .mp3 file");
          target.innerHTML = "Please upload .jpg, .png, .gif, .glb, .gltf, .4ds  .webp, .mp4 or .mp3 file";
        }else {
...
function themeSwitcher(selector){
    ...
    themeToggler.forEach(item => {
      item.addEventListener("click", function(e){
        ...
        if(document.body.classList.contains("dark-mode")) {
          sessionStorage.setItem("website_theme", "dark-mode");
        }else {
          sessionStorage.setItem("website_theme", "default");
        }
      });
    })
  }
  ...
  function retrieveTheme(){ 
    var theme = sessionStorage.getItem('website_theme');
    if(theme != null){
