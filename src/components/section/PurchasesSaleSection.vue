                    ...
                    <div class="profile-setting-panel">
                        <div class="history-panel" v-for="item in SectionData.purchaseAndSaleData.purchaseData" :key="item.id">
                            <span class="history-panel-left">
                                ...
                                <span>You bought <router-link to="/collections/1" class="btn-link fw-medium">{{ item.title }}</router-link> from <router-link :to="item.authorLink" class="btn-link fw-medium">{{ item.author }}</router-link> for <span class="fw-medium">&dollar;{{ item.price }}</span></span>
                            </span>
                ...
                <div class="tab-pane fade" id="sales" role="tabpanel" aria-labelledby="sales-tab">
                    <div class="profile-setting-panel">
                        <div class="history-panel" v-for="item in SectionData.purchaseAndSaleData.salesData" :key="item.id">
                            <span class="history-panel-left">
                                <em class="ni ni-money me-1"></em>
                                <span><router-link :to="item.authorLink" class="btn-link fw-medium">{{ item.author }}</router-link> bought <router-link to="/collections/1" class="btn-link fw-medium">{{ item.title }}</router-link> You for <span class="fw-medium">&dollar;{{ item.price }}</span></span>
                            </span>
                            <span class="history-panel-right">{{ item.dateText }}</span>
                        </div><!-- end history-panel -->
                    </div><!-- end profile-setting-panel -->
